...
class DataStoreAppSettings(context: Context) : AppSettings {
    ...
    override val preferences: Flow<UserPreferences> = dataStore.data
        ...
        .map { store ->
            ...
            UserPreferences(
                ...
                liveSessionBackend = store[Keys.LIVE_SESSION_BACKEND]
                    ?.let(::parseLiveSessionBackend)
                    ?: defaults.liveSessionBackend,
                realtimeWebSocketUrl = store[Keys.REALTIME_WEBSOCKET_URL] ?: defaults.realtimeWebSocketUrl,
    ...
    override suspend fun update(prefs: UserPreferences) {
        dataStore.edit { store ->
            ...
            store[Keys.LIVE_SESSION_BACKEND] = prefs.liveSessionBackend.name
            store[Keys.REALTIME_WEBSOCKET_URL] = prefs.realtimeWebSocketUrl
    ...
    private fun parseLiveSessionBackend(raw: String): LiveSessionBackend {
        val defaults = UserPreferences()
        return LiveSessionBackend.entries.firstOrNull { it.name == raw } ?: defaults.liveSessionBackend
    }
    ...
    private object Keys {
        ...
        val LIVE_SESSION_BACKEND = stringPreferencesKey("live_session_backend")
        val REALTIME_WEBSOCKET_URL = stringPreferencesKey("realtime_websocket_url")
